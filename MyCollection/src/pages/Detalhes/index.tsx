...
import {Factory, useToast} from 'native-base';
...
export const Detalhes: React.FC = () => {
  ...
  const isMyne = useMemo(() =>!!data?historico.map(itemHistorico=>itemHistorico.jogoId).includes(data.id):false, [historico, data])
    const addCart = () => {
        if (data) {
            addItem({
                jogoId: data.id,
